...
class VeloSource(BaseSource):
    ...
    def _fetch_batch(self, exchange: str, coins: list, begin_ts: int, end_ts: int) -> list:
        ...
        try:
            ...
            for line in lines[1:]:
                ...
                # Look up entity_id
                entity_id = self._entity_cache.get(coin)
    ...
    def _insert_metrics(self, records: list) -> int:
        ...
        # Prepare rows: (pulled_at, source, asset, entity_id, metric_name, value, domain, exchange)
        rows = [
            (
                r['pulled_at'], 
                self.source_name, 
        ...
        # Upsert query with new columns
        query = """
            INSERT INTO metrics (pulled_at, source, asset, entity_id, metric_name, value, domain, exchange)
            VALUES %s
